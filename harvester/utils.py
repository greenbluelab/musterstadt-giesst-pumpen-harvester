...
def transform_dataframe(gdf):
    ...
    cleaned_gdf = gdf.filter(["id", "name", "drinking_water", "addr:full", "pump:style", "check_date","pump:status", "geometry"])
    ...
    if "pump:status" not in cleaned_gdf:
        cleaned_gdf["pump:status"] = pd.Series(dtype=str)
    ...
    cleaned_gdf["pump:status"] = cleaned_gdf["pump:status"].replace("ok", "funktionsfähig")
    cleaned_gdf["pump:status"] = cleaned_gdf["pump:status"].replace("locked", "verriegelt")
    cleaned_gdf["pump:status"] = cleaned_gdf["pump:status"].replace("blocked", "verriegelt")
